...
function findRealAxes(num, el) {
  const ret = [];
  if (num === 0) {
    ret.push(numberToEl(num, el));
  } else {
    const axes = el.axes[num-1];
    let findAxes = false;
    for (let i=num; i < el.realAxes.length; i++) {
      if (el.realAxes[i].el === axes) {
	findAxes = true;
      }
      if (findAxes) {
	if (el.realAxes[i].type === 'revolute' ||
	    el.realAxes[i].type === 'continuous') {
	  break;
	}
	ret.push(el.realAxes.el);
      }
    }
  }
  return ret;
  ...
  tick: function() {
    if (this.el.workerData?.current?.status) {
      ...
      if (collisionPairs.length === 0) {
	    ...
	    findRealAxes(i, this.el)
	      .forEach(linkEl => changeLinksColor(linkEl, 'original'));
          }
          this.colored = false;
        }
        ...
        for (let i = 0; i < this.el.axes.length + 1; i++) {
          // const linkEl = numberToEl(i, this.el);
	  const links = findRealAxes(i, this.el);
          // globalThis.__customLogger?.debug('Status: Processing link index:', i,
          //          'link:', linkEl);
          if (!uniqueFlat.includes(i)) {
            // non-collision: original color
            links.forEach(linkEl=>changeLinksColor(linkEl, 'original'));
          } else {
            // collision
            links.forEach(linkEl=>changeLinksColor(linkEl, this.data.color));
            this.colored = true;
          }
        }
      }
